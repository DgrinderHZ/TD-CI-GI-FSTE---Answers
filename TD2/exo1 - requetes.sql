...
-- Note: NVL(<attribut>, <valeurs de remplacement>) remplacer les valeurs NULL
...
-- 2. Sélection des lignes de V_EMP oé le salaire total est supérieur é 10.000 DH
SELECT *
FROM V_EMP
WHERE gains > 10000;

-- 3. Essayez de mettre é jour le nom de l'employé MARTIN é travers la vue V_EMP.
-- Cette opération n'est pas possible directement via une vue avec des jointures.
UPDATE V_EMP
SET nom = 'Martin'
WHERE nom = 'test';
...
-- Essayez de modifier le département d'un employé visualisé é l'aide de cette vue.
-- Cette opération n'est pas possible directement via une vue sans respecter la condition
-- où le numéro du département est égal à 10.
UPDATE VEMP10
SET n_dept = 30
WHERE nom = 'Test2';
...
-- 7.Vous pourrez chercher une autre solution qui n'utilise pas de vue mais un select emboété dans le from.
SELECT e.nom, e.salaire, 
       (e.salaire / t.total_salaire_dept * 100) AS pourcentage_salaire
FROM (
    SELECT n_dept, SUM(salaire) AS total_salaire_dept
...
-- 7. BONUS: Credits to IBRAHIME LAMRABET
SELECT e.nom, e.salaire, 
       (e.salaire / ( SELECT SUM(salaire) FROM emp WHERE n_dept = e.n_dept) * 100) 
       AS pourcentage_salaire
FROM emp e;
